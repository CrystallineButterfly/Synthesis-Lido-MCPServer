...
contract LidoOperatorRegistryTest is Test {
    LidoOperatorRegistry private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_lido_mcpserver");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new LidoOperatorRegistry(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        LidoOperatorRegistry freshController = new LidoOperatorRegistry(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
